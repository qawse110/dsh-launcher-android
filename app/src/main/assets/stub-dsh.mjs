...
try {
  // Android 无桌面 opener：dsh-host-apiproxy 的平台分发缺 android 分支，
  // 点击「打开路径」直接抛 native path opener is unsupported。
  // 这里注入两处：open 分支走 Termux 的 termux-open；canOpen 对 android 放行
  // （按钮可见，缺失 termux-open 时点击会得到明确的 ENOENT 错误而非静默）。
  const MARKER_NPO = 'dsh-launcher-android-native-path-opener';
  const apiproxy = findPkg('@deepseek-ai/dsh-host-apiproxy', 'lib/index.js');
  if (!apiproxy) {
    log('dsh-host-apiproxy: not found, skip native path opener patch');
  } else {
    let src = readFileSync(apiproxy, 'utf8');
    if (src.includes(MARKER_NPO)) {
      log('native path opener already patched');
    } else {
      const OPEN_ANCHOR = '\tif (platform === "linux") {\n\t\tif (wsl) {';
      const OPEN_NEW = [
        '\tif (platform === "android") {',
      ...
      if (!src.includes(OPEN_ANCHOR) || !src.includes(CAN_OLD)) {
        log('native path opener anchors not found, skip（上游结构变化）');
      } else {
        src = src.replace(OPEN_ANCHOR, OPEN_NEW).replace(CAN_OLD, CAN_NEW);
        const patched = '/* ' + MARKER_NPO + ' */\n' + src;
        // 写盘前语法校验，防止毒化
        const chk = join(dirname(apiproxy), '.npo.check.mjs');
        writeFileSync(chk, patched);
        const vr = spawnSync(process.execPath, ['--check', chk], { timeout: 15000 });
        rmSync(chk, { force: true });
        if (vr.status !== 0) {
          log('WARN native path opener patch failed syntax check, abort');
        } else {
          writeFileSync(apiproxy, patched);
          log('native path opener patched: termux-open branch + canOpen(android)');
        }
      }
    }
  }
} catch (e) {
  log('WARN native path opener: ' + (e.stack || e.message));
}

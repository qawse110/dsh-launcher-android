 ...
 */
import { writeFileSync, existsSync, readdirSync, readFileSync, mkdirSync, unlinkSync, chmodSync } from 'node:fs';
...
/**
 * pnpm 从 store 硬链接落盘，store 里的文件是 0444，node_modules 里的硬链接共享
 * 同一 inode —— 直接 writeFileSync 会 EACCES，而外层 try/catch 只打一行 WARN，
 * 补丁静默失效、启动期才炸。这里先 chmod，仍失败就删掉重写（新文件默认 0644）。
 */
function writeFileForce(p, data) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`${e2.code || e2.message} (first: ${e1.code || e1.message})`);
    }
  }
}

/**
 * 与 findPkg 的区别：返回**所有**命中副本。
 * 只打第一个副本是不够的——.pnpm 里可能同时存在 sharp@0.33.x 与 sharp@0.35.x
 * （多个依赖各锁一版），readdir 顺序决定谁被打补丁，加载方解析到的可能是另一份。
 */
function findPkgAll(pkgName, rel) {
  const out = [];
  const seen = new Set();
  ...
  push(join(NODE_MODULES, pkgName, rel));
  // npm 嵌套布局：可能散落在任意包的 node_modules 里
  try {
    const nested = findNestedPkgAll(pkgName, rel);
    for (const p of nested) push(p);
  } catch {}
  return out.filter((p) => existsSync(p));
}

/** 递归收集嵌套 node_modules 中的全部命中路径（findNestedPkg 的单命中版本）。 */
function findNestedPkgAll(pkgName, rel) {
  const found = [];
  function walk(dir, depth) {
    if (depth > 8) return;
    let entries;
...
try {
  ...
  const shimBuf = Buffer.from(SHIM_B64, 'base64');

  /* 第二道防线：把同一份 shim 落到 $HOME/sharp-shim.cjs。fs-loader.mjs 会在
     模块解析期把裸说明符 `sharp` 重定向到它 —— 覆盖本补丁失效的全部场景
     （多副本漏打 / pnpm 重新 materialize 覆盖 / 只读硬链接写失败 / stub 被
     marker 跳过）。放在最前面写，保证后续任何异常都不会连它一起带走。 */
  const SHIM_FILE = join(HOME, 'sharp-shim.cjs');
  try {
    writeFileForce(SHIM_FILE, shimBuf);
    log('sharp runtime shim: ' + SHIM_FILE);
  } catch (e) {
    log('WARN sharp runtime shim write: ' + e.message);
  }
  ...
  for (const [pkg, rel, req] of targets) {
    // 遍历全部副本，而不是 findPkg 的「第一个命中」
    for (const p of findPkgAll(pkg, rel)) {
      ...
      try {
        if (!writtenShims.includes(shimAbs)) {
          writeFileForce(shimAbs, shimBuf);
        ...
        writeFileForce(p, payload);
        log('sharp shim ok: ' + p);
        n++;
      } catch (e) {
        failed++;
        log('WARN sharp shim failed: ' + p + ' -> ' + e.message);
      }
    }
  }
  if (n === 0 && failed === 0) log('sharp: not found, skip');

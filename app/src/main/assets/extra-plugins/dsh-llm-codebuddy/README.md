...
**两区域的模型体系是各自独立的**，不是同一份清单换域名：中国区以混元 / DeepSeek /
GLM 为主，国际版以 GPT / Gemini 为主，只有少数同名项（`glm-5.3`、`glm-5.2`、
`kimi-k2.6`、`minimax-m3`）。因此插件的目录获取逻辑相同（同一个
`GET /v3/config`，同一个 `cli.models` 解析），但拿到的模型列表完全不同。

### 国际版自动补齐国内版模型

少数模型在**国际版端点可用**、也确实列在**国内版目录**里，却没有进国际版的
`cli.models` 白名单，因此按白名单过滤时会整个消失。插件在拉取国际版目录后会
**额外拉一次国内版目录**，把这些模型按国内版规格补齐：

| 补齐的模型 | 国际版端点实测 | 说明 |
|---|---|---|
| `hy4-preview` | 200 OK | 思考型（`onlyReasoning`），输出走 reasoning |
| `hy4-preview-f` | 200 OK | credit 0（免费） |
| `deepseek-v4.1-flash` | 200 OK | credit 0（免费），默认思考档 high |
| `hy3` | 200 OK | credit 0（免费），默认思考档 high |

补齐走的是国内版在线目录的**同一套解析逻辑**，所以上下文窗口、最大输出与
**思考等级**（`thinkingLevelMap` / `defaultReasoningEffort`）与国内版完全一致；
`baseUrl` 仍指向国际版，请求不会误发到国内。国内版目录不可达时静默跳过，
只显示国际版自有模型。

> [!NOTE]
> 该白名单（`CROSS_REGION_MODEL_IDS`）需要跟着**国内版实时目录**走，不能照抄历史
> 快照：实测 `hy4-preview-x` 已从国内版目录下线，取而代之的是 `hy4-preview-f`；
> `hy3-x` 在国际版端点不可用（400），故未列入。缺失的 id 会自动跳过，不会报错。
...
- **用量查询**：设置页新增「CodeBuddy 用量」页面，读取与 CodeBuddy 个人中心
  ...
  并含**积分消耗明细**与**会话用量统计**两个视图，支持中国区与国际版切换；
- **积分消耗明细**：逐条请求 × 模型 × 客户端 × 积分，按模型 / 按日 / 按客户端聚合
  （官方 `get-user-request-usage` 接口，按日分片拉取以规避服务端 3000 条上限）；
- **会话用量统计**：扫描本地会话日志，按模型与会话聚合真实 token 用量
  （输入 / 输出 / 缓存读），覆盖所有 Provider，不依赖 CodeBuddy 接口；
- 独立安装，不修改 DSH 全局安装目录。
...
设置页的「CodeBuddy 用量」页面有三个并列视图：**中国区**、**国际版**、
**积分消耗明细**。前两个展示账号额度，第三个展示逐条消耗记录。
...
与个人中心「使用明细」表格同源，逐条给出 `requestId / 模型 / 客户端 / 积分 / 时间`，
在插件侧聚合成三张表：按模型、按日、按客户端。

```text
POST /billing/meter/get-user-request-usage     —— 逐条请求的积分消耗
     请求体 { startTime, endTime, pageNum, pageSize }（时间格式 "YYYY-MM-DD HH:mm:ss"）
```

> [!IMPORTANT]
> 服务端单次查询的 `total` **硬封顶 3000 条**：整月/跨月区间一律只返回 3000 条，
> 而实测 9 月逐日合计超过 3900 条——宽区间查询会**静默丢数据**。
> 因此本插件按**自然日分片**逐日拉取再合并（按 `requestId` 去重），
> 返回的 `fetched` 才是真实条数，`reportedTotal` 仅是该分片的最大值。
> 分片拉取约 30 次请求、耗时 10-20 秒，属正常。

### 会话用量统计

扫描本地 DSH 会话日志（`$DSH_HOME/sessions/*/*/session.jsonl.zstd`），按模型与会话
聚合真实 token 用量（输入 / 输出 / 缓存读 / 缓存写）与请求次数。与计费接口互补：
官方接口只有积分，本地日志才有 token 粒度，且能覆盖所有 Provider（不限 CodeBuddy）。

两个实现要点：

- 会话日志是**流式追加的多帧 zstd**（单文件可达上万帧），
  `zlib.zstdDecompressSync` 遇到多帧输入只解第一帧就返回，必须按帧边界逐帧解压；
- 全量冷扫约 12 秒纯 CPU，放在 **worker 线程**里跑，避免阻塞宿主事件循环
  （否则会卡住 LLM 流式与其它 web 路由）；按文件 mtime 缓存，重复查询近乎零开销。

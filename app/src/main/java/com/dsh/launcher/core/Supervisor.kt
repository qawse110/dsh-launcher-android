...
object Supervisor {
    ...
    fun reviveWebIfDue(ctx: Context): Boolean {
        ...
        //
        //   dsh 启动顺序是：先加载整棵插件树（实测 index 103 个条目，数十秒），
        //   **最后**才 listen 3080。于是从「流程开始」到「端口 bind」之间有
        //   数十秒窗口，此间 isUp() 恒为 false。watchdog 每轮探测都判「web 挂了」
        //   → reviveWebIfDue 再拉一个 node → 两进程先后 bind 3080 →
        //   后到者 EADDRINUSE 崩溃 → 主流程 waitForWebReady 白等 90 秒。
        //
        ...
        // 端口已监听但 HTTP 无响应＝残留进程占着：此时再拉只会 EADDRINUSE。
        // 交给 DshFlow 的启动路径（它会在启动前 killAllNode 清场）处理。

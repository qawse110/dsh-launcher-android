...
- **交互**：**单击**桌宠本体——挥手互动一次 + 随机台词短气泡（5 秒，会朗读）；
  ...
  **快速连点 ≥5 次**触发"主人别戳啦～"吐槽；无临时气泡时点击**气泡**打开 dsh Web，
  桌宠模式的**空白区域点击无反应**；**拖动松手后桌宠沿抛出方向做抛物线坠落**，撞左右墙
  反弹、落地小弹跳，随后**自动走回屏幕侧边待机**（抬升避开底部导航，不再停在底部挡
  操作；设置页「拖拽抛落」可关，改回普通拖动定位）；**关闭悬浮窗**：拖动时屏幕底部
  出现垃圾桶，把悬浮窗拖到垃圾桶上松手即关闭（「悬浮窗显示」重新开启后恢复），
  原右上角 × 按钮已移除；长按切换模式。
...
- **外观**：设置页可调桌宠大小（小/中/大）、气泡开关与**桌宠名称开关**（气泡中是否显示
  ...
  自动换行截断；气泡靠左排布，**气泡变宽不影响桌宠本体位置**；精灵表按单元格像素
...
- **发声（TTS）**：设置页「桌宠发声」开关（默认开）。任务完成、出错、收到新任务、
  调用工具时会语音播报固定台词；点击互动时朗读台词（同一状态只播报一次，间隔 ≥4 秒
  防反弹连播）。使用系统 TTS 引擎：优先中文，无中文引擎自动回退系统默认语言，
  无引擎时静默不报错。
